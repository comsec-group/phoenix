...
class BaseSoC(SoCCore):
    def __init__(self, *, sys_clk_freq=int(50e6), iodelay_clk_freq=200e6,
        ...
        platform = antmicro_ddr5_test_board.Platform()

        # SoCCore ----------------------------------------------------------------------------------
        SoCCore.__init__(self, platform, sys_clk_freq,
            ident = "LiteX SoC on DDR5 Test Board",
            **kwargs)
        ...
        if not self.integrated_main_ram_size:
            self.submodules.ddrphy = ddr5.K7DDR5PHY(platform.request("ddr5"),
                iodelay_clk_freq = iodelay_clk_freq,
                sys_clk_freq     = sys_clk_freq,
            )
            self.add_sdram("sdram",
                phy                     = self.ddrphy,
                module                  = MT60B2G8HB48B(sys_clk_freq, "1:8"),
...
def main():
    ...
    parser = LiteXSoCArgumentParser(description="LiteX SoC on DDR5 Test Board")
