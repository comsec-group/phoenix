...
from litex_boards.platforms import antmicro_datacenter_ddr4_test_board_v12
...
class BaseSoC(SoCCore):
    def __init__(self, *, sys_clk_freq=100e6, iodelay_clk_freq=200e6,
            ...
            **kwargs):
        platform = antmicro_datacenter_ddr4_test_board_v12.Platform()

        # UNSUED -----------------------------------------------------------------------------------
        diode_y = platform.request("hot_swap_yellow")
        diode_r = platform.request("hot_swap_red")
        diode_g = platform.request("hot_swap_green")
        self.comb += diode_y.eq(0)
        self.comb += diode_r.eq(0)
        self.comb += diode_g.eq(0)
        pw_cycle = platform.request("fpga_power_cycle")
        self.comb += pw_cycle.eq(0)
        fpga_slp_s4 = platform.request("fpga_slp_s4")
        self.comb += fpga_slp_s4.eq(0)
        fpga_vtt_cntl = platform.request("fpga_vtt_cntl")
        self.comb += fpga_vtt_cntl.eq(0)
        ...
        if not self.integrated_main_ram_size:
            self.ddrphy = K7DDRPHY(platform.request("ddr4"),
                memtype                   = "DDR4",
...
def main():
    ...
    parser = LiteXArgumentParser(platform=antmicro_datacenter_ddr4_test_board_v12.Platform, description="LiteX SoC on DDR4 Datacenter Test Board.")
