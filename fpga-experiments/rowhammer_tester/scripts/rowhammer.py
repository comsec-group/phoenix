    ...
    def display_errors(self, row_errors, _read_count, do_error_summary=False):
        ...
        for row in row_errors:
            ...
            if len(row_errors[row]) > 0:
                flips = sum(self.bitflips(value, expected) for addr, value, expected in row_errors[row])
                print("Bit-flips for row {:{n}}: {}".format(row, flips, n=len(str(2**self.settings.geom.rowbits - 1))))
                for addr, value, expected in row_errors[row]:
                    print(f"0x{addr:08x}", f"0x{(value^expected):x}", sep=',')
