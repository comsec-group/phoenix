...
def RemoteClient(*args, **kwargs):
    ...
    class RemoteClientWrapper:
        def __init__(self, *args, **kwargs):
            self.client = _RemoteClient(csr_csv=get_generated_file('csr.csv'), *args, **kwargs)
        ...
        def __getattr__(self, name):
            # Forward attribute access to the underlying client object
            return getattr(self.client, name)

    return RemoteClientWrapper(*args, **kwargs)
...
def hw_memset(wb, offset, size, patterns, dbg=False, print_progress=True, sleep_time=10e-3):
...
class BISTError:
    ...
    def __repr__(self):
        return f"BISTError(offset={self.offset}, data={hex(self.data)}, expected={hex(self.expected)})"


def hw_memtest(wb, offset, size, patterns, dbg=False, print_progress=True) -> List[BISTError]:
    # we are limited to multiples of DMA data width
    settings = get_litedram_settings()
    dma_data_width = settings.phy.dfi_databits * settings.phy.nphases
    dma_data_bytes = dma_data_width // 8

    assert size % dma_data_bytes == 0, (
        f"Memory test size ({size} bytes) must be a multiple of the DMA data width in bytes ({dma_data_bytes} bytes, "
        f"derived from DMA data width of {dma_data_width} bits). Please adjust the size to satisfy this requirement."
    )
    assert len(patterns) == 1  # FIXME: Support more patterns
    ...
    # FIXME: Support more patterns
    wb.write(wb.mems.writer_pattern_data.base, [pattern] * (dma_data_bytes // 4))  # pattern is 32-bit
    wb.write(wb.mems.writer_pattern_addr.base, offset // dma_data_bytes)
    wb.write(wb.mems.reader_pattern_data.base, [pattern] * (dma_data_bytes // 4))  # pattern is 32-bit
    wb.write(wb.mems.reader_pattern_addr.base, offset // dma_data_bytes)
    # Unmask just one pattern/offset (will always take data/addr from address 0)
    wb.regs.reader_data_mask.write(0x00000000)

    count = size // dma_data_bytes
...
def hw_memtest_count(wb, offset, size, patterns, dbg=False) -> int:
    # we are limited to multiples of DMA data width
    settings = get_litedram_settings()
    dma_data_width = settings.phy.dfi_databits * settings.phy.nphases
    dma_data_bytes = dma_data_width // 8
    ...
    while True:
        if wb.regs.reader_ready.read():
            count = wb.regs.reader_error_count.read()
            break
        time.sleep(10e-3)  # !0 ms

    # while True:
    #     if not wb.regs.reader_error_ready.read():
    #         count = wb.regs.reader_error_count.read()
    #         break
    #     time.sleep(10e-3)  # !0 ms

    assert wb.regs.reader_ready.read() == 1
    # assert wb.regs.reader_error_ready.read() == 1
    # count_after = wb.regs.reader_error_count.read()
...
def execute_payload(payload: List[int], wb, verbose: bool = False):
    if verbose:
